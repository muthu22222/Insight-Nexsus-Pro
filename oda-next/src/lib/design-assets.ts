...
const STYLE_ROOM_IMAGES: Record<string, [string, string, string]> = {
  ...
  luxury: [
    ...
    'https://images.unsplash.com/photo-1600210492486-724fe5c67fb0?w=1600&auto=format&fit=crop&q=85', // Bespoke interior with full seating suite and designer lighting
...
export function getDesignImagesForStyle(styleName?: string, roomTypeName?: string): [string, string, string] {
  ...
  // Match specific styles (traditional, scandinavian, luxury, industrial, minimalist, contemporary)
  for (const styleKey of ['traditional', 'scandinavian', 'luxury', 'industrial', 'minimalist', 'contemporary', 'modern']) {
    if (normalizedStyle.includes(styleKey)) {
      return STYLE_ROOM_IMAGES[styleKey];
    }
  }
